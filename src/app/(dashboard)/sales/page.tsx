...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return createPageMetadata(t("salesTitle"), t("salesDescription"));
}

export default async function SalesInvoicesPage() {
  const session = await auth();
  if (!session?.user) redirect("/login");
  if (!hasPermission(session.user.permissions, "stockOutView")) return <Unauthorized />;
  const canCreate = hasPermission(session.user.permissions, "stockOutCreate");
  const t = await getTranslations("invoices");

  const [parties, products, batches] = await Promise.all([
    prisma.party.findMany({
      where: { isActive: true, type: { in: [PartyType.CUSTOMER, PartyType.BOTH] } },
  ...
  const batchRows = batches.reduce<Record<string, Array<{ id: string; productId: string; batchNumber: string; expiryDate: string | null; currentQuantity: string; purchaseCost?: string | null }>>>((acc, batch) => {
    ...
    if (currentQuantity.lte(0)) return acc;
    acc[batch.productId] ??= [];
  ...
  return (
    <PageShell>
      <AppPageHeader title={t("salesTitle")} description={t("salesDescription")} />
      <InvoiceWorkspace
        mode="sales"
        title={t("salesWorkspaceTitle")}
        description={t("salesWorkspaceDescription")}
